...
export function AnimatedBackground() {
  ...
  useEffect(() => {
    ...
    // Определяем количество частиц в зависимости от размера экрана и производительности
    const getParticleCount = () => {
      const width = window.innerWidth;
      const isMobile = width < 768;
      const isTablet = width >= 768 && width <= 1024;
      
      // Меньше частиц на мобильных для экономии ресурсов
      if (isMobile) return 15;
      if (isTablet) return 25;
      return 40; // Уменьшено с 50
    };
    ...
    const getOrbCount = () => {
      ...
      if (isMobile) return 1; // Минимум на мобильных
      if (width > 1024) return 4; // Уменьшено с 5
      return 2;
    };
